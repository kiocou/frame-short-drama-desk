...
def load_local_config() -> Dict[str, Any]:
    ...
    config_path = Path(os.path.expandvars(configured)).expanduser() if configured else get_runtime_base_dir() / "config.json"
...
def get_media_work_dir() -> Path:
    """Keep desktop media under the user's Videos folder, not beside the app."""
    configured = os.getenv("DUANJU_DOWNLOAD_DIR", "").strip()
    output_root = Path(os.path.expandvars(configured)).expanduser() if configured else Path.home() / "Videos" / "短剧下载"
    work_dir = output_root.resolve() / ".parts"
...
def curl_request(
    ...
    if post_body is not None:
        resp = get_http_session().post(url, headers=headers, data=post_body, timeout=timeout)
    else:
        resp = get_http_session().get(url, headers=headers, timeout=timeout)
                    ...
                    if isinstance(video_list, dict):
                        ...
                        if best_key and best_item:
                            ...
                            if raw_main_url:
                                ...
                                try:
                                    local_url = download_decrypt_and_serve(request, real_main_url, content_key)
                                    if local_url:
...
def stream_copy_video_with_ffmpeg(
    ...
    # Web responses are temporary. Desktop downloads must survive until the
